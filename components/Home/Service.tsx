...
const ServiceHeader = ({ section, title, description }) => {
  return (
    <div className="lg:text-center p-4 md:p-0">
      {/* <p className="uppercase font-semibold text-themecolor">{section}</p> */}
      <h2 className="text-3xl leading-8 font-extrabold tracking-tight text-themecolor-bright">
        {title}
      </h2>
      <p className="mt-4 text-2xl leading-7 text-description lg:mx-auto">
        {description}
      </p>
    </div>
  );
  ...
  return (
    <div className="flex space-x-4 auto-padding">
      ...
      <div className="my-2 lg:my-4">
        ...
        <p className="leading-normal text-themecolor-bright text-xl">{description}</p>
      </div>
    </div>
  );
};
